...
run against the same council website, separately from the seed+depth crawl
this branch's `scripts/scrape.py` uses.
...
**actual URLs** visited and categorised, which is exactly the input needed
to check one crawl's coverage against the other.

- `db-unza26-csc4792-luanshya_council_crawled_pages.csv` — 68 HTML pages
  crawled, with title, crawl depth, HTTP status, and keyword-based category
  tags.
